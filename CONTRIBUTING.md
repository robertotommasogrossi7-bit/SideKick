# Contributing

Thanks for stopping by. The most useful contributions right now, in order:

1. **Join the with/without study.** The core hypothesis (process-scaffolding helps the
   *human*, not the expert model) is designed but has no test subjects yet — see
   [FINDINGS.md](FINDINGS.md). If you'd like to participate in a small study,
   **open an issue**.
2. **Attack the data or the claims.** If a number in [`observatory/`](observatory/DATA.md)
   doesn't add up, or a claim isn't supported by the stated N, open an issue — this repo's
   whole point is being verifiable. (Two external AI red-team rounds are already public in
   [`observatory/redteam/VERDICTS.md`](observatory/redteam/VERDICTS.md).)
3. **Improve the tools.** Both [`observatory/usage.mjs`](observatory/usage.mjs) and
   [`experiments/cost-meter.mjs`](experiments/cost-meter.mjs) have fixture-based tests in
   [`tests/`](tests/) and CI runs them on every push. PRs with a test are easiest to merge.

Issues and PRs are welcome **in English or Italian**. An Italian translation of this guide
lives in [`ITALIANO/CONTRIBUTING.md`](ITALIANO/CONTRIBUTING.md); the legacy Italian docs
(pre-2026-07-25) are preserved in the repo's git history.
